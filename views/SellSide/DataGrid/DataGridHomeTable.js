...
class DataGridHomeTable extends Component {
  ...
  handleScroll() {
    if (this.props.anaylsisList !== null && this.props.savedAnalysisPaginationInfo.totalResultsCount !== this.props.anaylsisList.length && !this.props.inprocess) {
      // if (this.props.anaylsisList !== null && !this.props.inprocess) {
      let pointToCallApi = this.scrollRef.current.scrollHeight - (document.documentElement.offsetHeight - (this.scrollRef.current.offsetTop + 60));
      if (this.scrollRef.current.scrollTop === pointToCallApi) {
        this.props.analysisAPICall(100, this.props.savedAnalysisPaginationInfo.currentPage + 1, this.props.that);
      }
    }
  }

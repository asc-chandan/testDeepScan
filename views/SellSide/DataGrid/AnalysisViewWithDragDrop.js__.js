...
const move = (source, destination, droppableSource, droppableDestination) => {
  ...
  if(droppableDestination.droppableId==='filters' && (removed.type==='number' || removed.type==='calculated' || removed.title==='values' || removed.title==="date") ){
    return false;
  }
...
class AnalysisViewWithDragDrop extends Component {
  constructor(props) {
    super(props);
  ...
  getDataGrid(date_range){
    ...
    if(this.view_type==='webanalytics'){ columns_order = ['pageviews' , 'sessions', 'users'] }
    ...
    APIService.apiRequest(Constants.API_BASE_URL+'/getDataGrid', analysisPayLoad)
      .then(response => {
        if(response.status===1 && response.data!==undefined){
          ...
          this.setState({
            inprocess: false, 
            analysisColumns: Object.keys(response.data[0]),
  ...
  getAnalysisData(analysisPayLoad, request_send_time){
    ...
    APIService.apiRequest(Constants.API_BASE_URL+'/getAnalysisLData', analysisPayLoad, true)
      .then(response => {
        ...
        that.setState({
          total_content_length: parseInt(contentLength),
          data_requests_list: data_req_list
        });

        let date_reading_start_time = performance.now();
        ...
        response.reader.read().then(function processResult(result) {
          if (result.done) {
            ...
            //Show download completion time
            let data_reading_completion_time = performance.now();
            let data_req_list = JSON.parse(JSON.stringify(that.state.data_requests_list));
            let completion_time = (data_reading_completion_time-date_reading_start_time).toFixed(2);
            data_req_list.push(
              {id:3, name:'Data download', time: completion_time, status: 'completed'},
              {id:4, name:'Data rendering', time: 0, status: ''}
            );
            that.setState({
              data_requests_list: data_req_list
            });
            ...
            let analysisData = JSON.parse(results.data);
            // console.timeEnd('sight: getAnalysisData - API call');

            // console.time('sight: getAnalysisData - render');
            // change inprocess state to false
            // let delay = (contentLength > 1000) ? 10 : 250;
            
            data_req_list[3]['time'] = (performance.now()-data_reading_completion_time).toFixed(2);
            data_req_list[3]['status'] = 'completed';

            that.setState({
              show_data_loading: false,
              data_requests_list: data_req_list,
              analysisColumns: results.columns,
          ...
          //show download percentage on loading and second part should be expected time
          // console.log(`Received ${total} of ${contentLength} - ${Math.round((total*100)/contentLength)}%`);

          let completion_percentage = Math.round((total*100)/contentLength);
          that.setState({
            inprocess_percent: parseInt(completion_percentage),
            inprocess_number: parseInt(total)
          });
  ...
  onDragEnd = result => {
    ...
    if (source.droppableId === destination.droppableId) {
      ...
      //Count number of cells - values*number of columns dimension
      setTimeout(() => {
        this.checkColumnsCount();
        this.checkRowsCount();
      },0);
        ...
        <div id="app-sub-header">
          ...
          {this.state.selected_columns_count > 0 &&
            <div className="meta-info">
              <div className="info">Rows/Columns: <span>{this.state.selected_rows_count+'/'+this.state.selected_columns_count}</span></div>
            </div>
          }
          ...
          <div className="action-button-wrapper">
            ...
            {/* Save Analysis View */}
            {!this.state.isSavedView &&
              <button className="btn outline xs btn-save-view" onClick={this.toggleSaveAnalysisDrawer} title="Save View">Save View</button>
            }

            {/* Analysis Filters Button */}
            {!this.state.isSavedView &&
              <button className="btn outline xs btn-toggle-filters" onClick={this.toggleFiltersPanel} title="Filters">Filters</button>
            }
          </div>
        </div>
          ...
          <div className="odin-view odin-analysis clearfix" data-client="" data-type="advertiser" data-lastupdated="">
            {this.state.showSaveAnalysisDrawer && 
              <ClickOutsideListner onOutsideClick={() => this.setState({ showSaveAnalysisDrawer: false })}>
                ...
                />
                {/* <SideDrawer>{this.analysisSaveForm()}</SideDrawer> */}
              </ClickOutsideListner>
            } 
...
export default AnalysisViewWithDragDrop;
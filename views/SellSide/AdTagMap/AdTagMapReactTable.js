...
function Table({columns, data, client, props}) {
  ...
  function handleColEdit(event, row_index, col_index){
    ...
    if(props.user.organization_id!==1 && props.user.privileges.indexOf('REVENUE_SHARE_SETTINGS') < 0){
      return false;
    } 
  ...
  function getEditableInputField(col, row){
    ...
    if(col==='effective_start_date'){
      return (<div className="start-date form-group">
        <input type="date" name="txt-start-date" id="txt-start-date" className="field-control" value={startDate} placeholder="Start Date" onChange={(e) => onInputChange(e, 'effective_start_date')} />
      </div>)
    }

    if(col==='effective_end_date'){
      return (<div className="end-date form-group">
        <input type="date" name="txt-end-date" id="txt-end-date" className="field-control" value={endDate} placeholder="End Date" onChange={(e) => onInputChange(e, 'effective_end_date')} />
      </div>)
    }
            ...
            <tr>
              ...
              <td>
                <div className="start-date form-group">
                  <input type="date" name="txt-start-date" id="txt-start-date" className="field-control" value={newStartDate} placeholder="Start Date" onChange={(e) => onInputChange(e, 'effective_start_date', true)} />
                </div>
              </td>
              <td>
                <div className="end-date form-group">
                  <input type="date" name="txt-end-date" id="txt-end-date" className="field-control" value={newEndDate} placeholder="End Date" onChange={(e) => onInputChange(e, 'effective_end_date', true)} />
                </div>
              </td>
              <td>
              ...
              <td>
                <div className="action-buttons-wrapper">
                  <button className="btn outline xs btn-view" onClick={handleAddNewRecord}>{inprocess ? 'adding...' : 'add'}</button>
                </div>
              </td>
            </tr>
          }
...
const AaTagMapReactTable = (props) => {
...
export default AaTagMapReactTable;